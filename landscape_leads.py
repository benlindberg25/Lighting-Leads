...
════════════════════════════════════════════════════════════════════
  ...
  Residential properties only (single family, condo, townhouse).
  Uses Google Street View for real home photos (no bot-blocking).
  Generates photorealistic AI landscape lighting photos.
  Usage:
...
NORTHERN_NASSAU = [
    ...
    "Sands Point", "Flower Hill", "Plandome", "North Shore",
]
...
def is_northern_nassau(address: str) -> bool:
    """Return True if the address is in one of the Northern Nassau towns."""
    addr_lower = address.lower()
    return any(town.lower() in addr_lower for town in NORTHERN_NASSAU)
...
# ─────────────────────────────────────────────────────────────────
# REDFIN SCRAPER  (county-level – residential only)
# ─────────────────────────────────────────────────────────────────
HEADERS = {
    "User-Agent": (
...
class RedfinScraper:
    ...
    def search_county_csv(self, region_id: str, min_price: int, max_age_days: int) -> List[Dict]:
        ...
        params = {
            ...
            "status": 9,
            "uipt": RESIDENTIAL_UIPT,   # residential only
            "region_type": 5,
            "region_id": region_id,
            "num_homes": 350,
            "sf": "1,2,3,5,6,7",
        }
        log.info(f"  Fetching sold residential listings CSV (region_id={region_id})...")
        ...
        try:
            ...
            for row in reader:
                try:
                    ...
                    continue
            log.info(f"  {len(results)} residential homes found in CSV")
            return results
        except Exception as e:
    ...
    # ── Google Street View – PRIMARY photo source ─────────────────
    def search_street_view(self, address: str, api_key: str) -> Optional[bytes]:
        """Fetch exterior photo via Google Street View Static API.
        This is the PRIMARY image source — it reliably returns real
        home photos without bot-detection issues.
        Returns image bytes, or None if no imagery available.
        """
        if not api_key:
            return None
        try:
            ...
            if r.status_code == 200 and "image" in r.headers.get("content-type", ""):
                if len(r.content) > 5_000:
                    log.info("  ✓ Google Street View photo obtained!")
                    return r.content
    ...
    def search_bing_image(self, address: str) -> Optional[str]:
        """Search Bing Images for an exterior photo (fallback only)."""
...
class LandscapeRenderer:
            ...
            resp = self.client.chat.completions.create(
                ...
                messages=[{"role":"user","content":[
                    ...
                    {"type":"text","text":(
                        ...
                        "Be specific — this will be used to generate a realistic landscape lighting photograph."
                    )}
                ]}],
                max_tokens=300
            )
    ...
    def generate_rendering(self, house_description: str) -> Optional[bytes]:
        prompt = (
            f"A photorealistic photograph — NOT a 3D render, NOT CGI — of a luxury residential "
            f"home at dusk with professional landscape lighting. Captured with a DSLR camera, "
            f"35mm lens, high dynamic range, sharp details. "
            f"HOME: {house_description} "
            "LIGHTING: warm 2700K LED uplights washing the facade, amber LED path lights "
            "lining the driveway and front walkway, architectural spotlights highlighting "
            "columns and dormers, dramatic uplighting into mature trees, recessed step lighting, "
            "garden accent lights, warm amber glow from windows. "
            "ATMOSPHERE: deep twilight blue sky behind the house, rich shadows, "
            "magazine-quality real estate photography. Looks exactly like a real photograph "
            "taken by a professional architectural photographer at golden hour / dusk."
        )
        try:
            ...
            try:
                resp = self.client.images.generate(
                    model="dall-e-3",
                    prompt=(
                        f"Photorealistic photograph of a luxury home at twilight with landscape lighting. "
                        f"{house_description[:150]} "
                        "Warm LED uplighting on facade, amber path lights, tree uplighting, "
                        "deep blue dusk sky. Real photograph quality, not a rendering or 3D image."
                    ),
                    size="1792x1024",
                    quality="hd",
                    n=1,
...
class LeadGenerator:
    ...
    def process_home(self, prop: Dict) -> bool:
        ...
        # ── Photo acquisition: Street View first (bot-free) ──────
        log.info("  Fetching exterior photo...")
        img_bytes = None

        # 1. Google Street View (primary — reliable, no bot-blocking)
        if STREET_VIEW_KEY:
            log.info("  → Google Street View (primary)...")
            img_bytes = self.redfin.search_street_view(address, STREET_VIEW_KEY)
        else:
            log.info("  → No Street View key set. Add STREET_VIEW_KEY for real home photos.")

        # 2. Bing image search (fallback)
        if not img_bytes:
            log.info("  → Bing image search (fallback)...")
            ...
            if photo_url:
                ...
                if img_bytes:
                    try:
                        ...
                        w, h = img.size
                        if w < 300 or h < 200:
                            log.warning(f"  Image too small ({w}x{h}) – skipping")
                            img_bytes = None
        ...
        # 3. No photo available – will generate from property data only
        if not img_bytes:
            log.info("  → No photo available – will generate from property description")
        ...
        if img_bytes:
            ...
            beds  = prop.get("beds", "")
            sqft  = prop.get("sqft", "")
            style = "grand estate" if price > 4_000_000 else "luxury"
            bed_str  = f"{beds}-bedroom, " if beds else ""
            sqft_str = f"{sqft} sq ft " if sqft else ""
            description = (
                f"A {style} single-family residential home in Long Island, NY. "
                f"{bed_str}{sqft_str}property with professional landscaping, "
                "manicured hedges, mature trees, a wide paved driveway, "
                "and elegant architectural details typical of high-end Nassau/Suffolk County homes."
            )
            log.info(f"  Description: {description[:100]}...")

        # ── DALL-E 3 photorealistic rendering ────────────────────
        log.info("  Generating photorealistic landscape lighting image with DALL-E 3...")
    ...
    def collect_properties(self) -> List[Dict]:
        ...
        log.info(f"\n{'='*60}")
        log.info("PHASE 1: SEARCHING FOR $1.3M+ SOLD RESIDENTIAL HOMES")
        log.info("Target: Northern Nassau County + All of Suffolk County")
        log.info(f"{'='*60}")
        ...
        for county in REDFIN_COUNTIES:
            ...
            # For Nassau County, restrict to Northern Nassau towns only
            if "nassau" in name.lower():
                before = len(props)
                props = [p for p in props if is_northern_nassau(p.get("address",""))]
                log.info(f"  Northern Nassau filter: {before} → {len(props)} homes")
        ...
        log.info(f"\nTotal qualifying residential homes found: {len(all_props)}")
    ...
    def run(self):
        ...
        log.info(f"  Target: Northern Nassau County + All of Suffolk County")
        log.info(f"  Property type: Residential only")
        if STREET_VIEW_KEY:
            log.info("  Photos: Google Street View API enabled ✓")
        else:
            log.info("  Photos: Set STREET_VIEW_KEY env var for real home photos")
        log.info(f"{'='*60}")
        ...
        if not properties:
            log.warning("\nNo qualifying homes found. Redfin may be blocking requests. Try again later.")
            return
